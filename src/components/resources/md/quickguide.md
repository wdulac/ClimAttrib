...
1. **Zoom-in** on the map to make the interactive grid appear and **click** on any grid cell to select a location.
3. Select the **date(s) of the event** using the calendar above the map, on the left.
4. Select the **event type** (*Hot / Cold*) using the *Extreme type* selector.
5. Choose whether or not to **focus on seasonal timing** using the *Seasonal context* selector (see note below).
6. **Click *Continue*** to proceed to the results page. Results may take from about twenty seconds up to several minutes to appear, depending on server load.
